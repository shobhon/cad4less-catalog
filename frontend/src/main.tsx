...
function findSocketViaNameValue(obj: any): string | null {
  ...
  while (stack.length) {
    ...
    if (
      typeof c.name === "string" &&
      c.name.toLowerCase().includes("socket")
    ) {
...
function isInStock(p: Part): boolean {
  ...
  // 1) If explicitly marked true, trust it.
  if (typeof anyPart.inStock === "boolean" && anyPart.inStock) {
    return true;
  }

  // 2) Look at vendor / offer lists for an "In stock" style availability.
  const vendorLists: any[] = [];
  if (Array.isArray(anyPart.vendorList)) vendorLists.push(...anyPart.vendorList);
  if (Array.isArray(anyPart.vendors)) vendorLists.push(...anyPart.vendors);
  if (Array.isArray(anyPart.offers)) vendorLists.push(...anyPart.offers);
  ...
  // 3) Fall back to top-level availability-style string fields.
  ...
  // If any vendor explicitly said "out of stock" and we don't see a positive signal, treat as not in stock.
  if (sawExplicitOutOfStock) {
    return false;
  }

  // If we have a clear "not available" pattern at the top level, respect it.
  if (availabilityRaw) {
    ...
    // Any other non-empty value (including "in stock", "available", "unknown") we treat as available.
    return true;
  }

  // 4) No information at all: default to showing the part.
  return true;
}
...
function getStoreLabel(p: Part): string {
  ...
  // Common nested vendor list shapes – prefer these over placeholder top-level values
  const vendorLists: any[] = [];
  if (Array.isArray(anyPart.vendorList)) vendorLists.push(...anyPart.vendorList);
  if (Array.isArray(anyPart.vendors)) vendorLists.push(...anyPart.vendors);
  if (Array.isArray(anyPart.offers)) vendorLists.push(...anyPart.offers);
  ...
  // Deep fallback: search nested structures (e.g. Apify / PcPartPicker vendor lists).
...
function applyFiltersAndSorting(
  ...
  const filtered = items.filter((p) => {
    ...
    // Text search across name, id and socket
...
function CatalogDashboard() {
  ...
  const cpuPageItems = cpuProcessed.sorted.slice(
    cpuStart,
    cpuStart + PAGE_SIZE
  );
    ...
    <div className="app-root">
      ...
      <section className="panel">
        <header className="panel-header">
          ...
          <p className="panel-subtitle">
            Internal dashboard for CPUs, motherboards, and CPU coolers fetched from the live CAD4Less backend.
          </p>
        </header>
      ...
      <section className="panel panel--catalog">
        <header className="panel-header">
          ...
          <p className="panel-subtitle">
            View all CPUs currently available in your catalog.
          </p>
        </header>
        ...
        <div className="table-meta">
          ...
          <span>
            Showing: {cpuPageItems.length} of {cpuMatching} (page {cpuPageClamped} / {cpuPageCount})
          </span>
        </div>
        ...
        {loadingCpus ? (
          ...
          <div className="table-empty">
            No CPUs found. Try changing the filters or running the CPU import job in the
            backend.
          </div>
        ) : (
          <>
            ...
            <div className="pagination">
              ...
              <span className="pagination-info">Page {cpuPageClamped} of {cpuPageCount}</span>
              <button
                type="button"
                className="btn"
                onClick={() =>
                  setCpuPage((prev) =>
                    Math.min(prev + 1, cpuPageCount)
                  )
                }
      ...
      <section className="panel panel--catalog">
        <header className="panel-header">
          ...
          <p className="panel-subtitle">
            View all motherboards currently available in your catalog.
          </p>
        </header>
        ...
        <div className="table-meta">
          ...
          <span>
            Showing: {mbPageItems.length} of {mbMatching} (page {mbPageClamped} / {mbPageCount})
          </span>
          {lastMbRefreshTime && (
            <span>
        ...
        {loadingMotherboards ? (
          ...
          <div className="table-empty">
            No motherboards found. Try changing the filters or running the
            motherboard import job in the backend.
          </div>
        ) : (
          <>
            <div className="table-wrapper">
            ...
            <div className="pagination">
              ...
              <span className="pagination-info">Page {mbPageClamped} of {mbPageCount}</span>
              <button
                type="button"
                className="btn"
                onClick={() =>
                  setMbPage((prev) => Math.min(prev + 1, mbPageCount))
                }
      ...
      <section className="panel panel--catalog">
        <header className="panel-header">
          ...
          <p className="panel-subtitle">
            View all CPU coolers currently available in your catalog.
          </p>
        </header>
        ...
        <div className="table-meta">
          ...
          <span>
            Showing: {coolerPageItems.length} of {coolerMatching} (page {coolerPageClamped} / {coolerPageCount})
          </span>
        </div>
        ...
        {loadingCoolers ? (
          ...
          <div className="table-empty">
            No CPU coolers found. Try changing the filters or running the cooler
          ...
          <>
            ...
            <div className="pagination">
              <button
                ...
                className="btn"
                onClick={() =>
                  setCoolerPage((prev) => Math.max(1, prev - 1))
                }
              ...
              <button
                ...
                onClick={() =>
                  setCoolerPage((prev) =>
                    Math.min(prev + 1, coolerPageCount)
                  )
                }
      ...
      <section className="panel">
        ...
        <div className="panel-body">
          <p>
            Placeholder for the build calculator. For now, use the three catalogs
            above to choose matching parts.
          </p>
        </div>
      </section>
    </div>
